...
function Home() {
  ...
  const handleReportSubmit = async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:5000${endpoint}?${selectedFilter}=${reportValue}&operator=${operator}`);
  ...
  const handleCityReportSubmit = async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:5000/cities/${selectedStateId}`);
